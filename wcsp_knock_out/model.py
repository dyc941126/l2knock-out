...
class GATNet(nn.Module):
    ...
    @torch.no_grad()
    def inference(self, x, edge_index, action_space_limit, function_idx):
        ...
        function_pooling = function_pooling.repeat((action_space_limit, 1))

        target = x[:action_space_limit, :]
        target = self.target_proj(target)
        out = F.elu(torch.cat([function_pooling, target], dim=1))
